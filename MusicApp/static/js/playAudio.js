...
function playSong(songSrc, songTitleText, artistText) {
    ...
    // Update the UI with the song title and artist
    songTitle.textContent = songTitleText + " | " + artistText;
...
function playNextTrack() {
    if (currentSongIndex === -1) { // If no song is playing
        ...
        if (currentSongIndex === 0 && !isShuffle) {  // If we hit the end, restart from the beginning in regular mode
...
function refreshPlaylistSongs(playlistId) {
    const playlistSongsDiv = document.querySelector('#playlist_songs');

    // Optionally: Add a loading indicator while fetching
    playlistSongsDiv.innerHTML = '<p>Loading...</p>';

    // Make an AJAX request to fetch the updated playlist songs
    fetch(`/app/playlist/${playlistId}/refresh_songs/`, {
        method: 'GET',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',  // Inform Django this is an AJAX request
        }
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        // Replace the current playlist_songs div with the updated content
        playlistSongsDiv.innerHTML = data;
    })
    .catch(error => {
        console.error('Error refreshing playlist songs:', error);
        playlistSongsDiv.innerHTML = '<p>Failed to load songs.</p>';
    });
}
...
function refreshFavouriteSongs() {
    ...
    // Make an AJAX request to fetch the updated playlist songs
    fetch(`/app/dashboard/favourites/refresh_songs/`, {
        method: 'GET',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',  // Inform Django this is an AJAX request
        }
